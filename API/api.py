import pymysql, json, configparser, os, datetime, base64, shutil
...
sslpath = path.split('API')[0] + 'API/ssl'
...
@cross_origin()
def account_Sign_up():
    pg = pymysql.connect(database = config['POSTGRES']['Account_db'], user = config['POSTGRES']['user'], password = config['POSTGRES']['password'], host = config['POSTGRES']['host'], port = int(config['POSTGRES']['port']))
    pgadmin = pg.cursor()
    data_json = {}
...
app.run(host='0.0.0.0', port = 6789, debug=True, ssl_context=(sslpath +'/certificate.crt', sslpath + '/private.key') )
